...
commit_info = defaultdict(lambda: {'name': '', 'commits': 0, 'last_commit': 0})

for commit in repo.iter_commits('master'):
    if commit.hexsha in last_commit:
        break
...
if not last_commit:
    ...
    c.execute('''CREATE TABLE committers
                 (name text, email text, commits integer, last_commit datetime)''')

    records = []
    for email in commit_info:
        info = commit_info[email]
        records.append((info['name'], email, info['commits'], info['last_commit']))
